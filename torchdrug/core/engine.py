...
class Engine(core.Configurable):
    ...
    def train(self, num_epoch=1, batch_per_epoch=None):
        ...
        for epoch in self.meter(num_epoch):
            ...
            # if epoch % 10 == 0:
            all_valid_loss_values.append(self.evaluate2())
            model.train()
            metrics = []
            start_id = 0
        ...
        while True:
            ...
            if not os.path.exists(csv_file_path):
                ...
                with open(csv_file_path, 'w', newline='') as csvfile:
                    fieldnames = ['epoch', 'all_train_loss_values', 'all_valid_loss_values']
                    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
                    writer.writeheader()


                    for epoch, t_loss, v_loss in zip(all_epoch_values, all_train_loss_values, all_valid_loss_values):
                        writer.writerow({'epoch': epoch, 'all_train_loss_values': t_loss, 'all_valid_loss_values': v_loss })  # Replace your_loss_value with the actual loss value
                    break  # Exit the loop
            file_number += 1
    ...
    def evaluate2(self, num_epoch=1, batch_per_epoch=None):
        """
        Evaluate the model.
        ...
        print(f'loss : {final_mean_loss.item()}')
